...
// Адаптация для страницы политики конфиденциальности
function initPolicyPage() {
    // Инициализируем только необходимые функции для страницы политики
    initMobileMenu();
    initSmoothScroll();
    initBackToTop();
    ...
    // Добавляем отступ для фиксированной шапки с учетом адаптивности
    const header = document.getElementById('header');
    const policySection = document.getElementById('policy');
    
    if (header && policySection) {
        const headerHeight = header.offsetHeight;
        policySection.style.paddingTop = headerHeight + 40 + 'px';
        
        // Обновляем при изменении размера окна
        window.addEventListener('resize', function() {
            policySection.style.paddingTop = header.offsetHeight + 40 + 'px';
        });
    }
}